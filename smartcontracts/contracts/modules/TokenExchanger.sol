...
contract TokenExchanger is BaseModule, RelayerModule, OnlyOwnerModule {
    ...
    // The address of the contract collecting fees for DuniaPay.
    address public feeCollector;
    // The DuniaPay fee in 1-per-10000.
        ...
        returns (uint256)
    {
        ...
        require(
            _srcToken == ETH_TOKEN_ADDRESS || _destToken == ETH_TOKEN_ADDRESS,
            "TE: source or destination must be ETH"
        );
        (uint256 destAmount, uint256 fee, ) = getExpectedTrade(
            _srcToken,
            _destToken,
            _srcAmount
        );
        ...
        returns (uint256 _destAmount, uint256 _fee, uint256 _expectedRate)
    {
        if (_srcToken == ETH_TOKEN_ADDRESS) {
            ...
            (_expectedRate, ) = KyberNetwork(kyber).getExpectedRate(
                ERC20(_srcToken),
                ERC20(_destToken),
                _srcAmount.sub(_fee)
            );
            ...
            _destAmount = _expectedRate.mul(_srcAmount.sub(_fee)).div(
                10**(36 - destDecimals)
            );
        } else {
            (_expectedRate, ) = KyberNetwork(kyber).getExpectedRate(
