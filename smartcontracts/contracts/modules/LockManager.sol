...
contract LockManager is BaseModule, RelayerModule {
    ...
    modifier onlyGuardian(BaseWallet _wallet) {
        (bool isGuardian, ) = GuardianUtils.isGuardian(
            guardianStorage.getGuardians(_wallet),
            msg.sender
        );
        require(
            msg.sender == address(this) || isGuardian,
            "GD: wallet must be unlocked"
        );
        _;
    }
     ...
     */
    function lock(BaseWallet _wallet)
        external
        onlyGuardian(_wallet)
     ...
     * @param _wallet The target wallet.
     */
    function unlock(BaseWallet _wallet)
        external
        onlyGuardian(_wallet)
     ...
     */
    function getLock(BaseWallet _wallet)
        public
        view
     ...
     */
    function isLocked(BaseWallet _wallet)
        external
        view
    ...
    function validateSignatures(
        ...
        (bool isGuardian, ) = GuardianUtils.isGuardian(
            guardianStorage.getGuardians(_wallet),
            recoverSigner(_signHash, _signatures, 0)
        );
