...
contract NewTestModule is BaseModule, RelayerModule, OnlyOwnerModule {
    ...
    function callDapp(address _wallet) external {
        invokeWallet(
            _wallet,
            address(dapp),
            0,
            abi.encodeWithSignature("noReturn()")
        );
    }
    ...
    {
        bytes memory result = invokeWallet(
            _wallet,
            address(dapp),
            0,
            abi.encodeWithSignature("uintReturn(uint256)", _val)
        );
        if (_isNewWallet) {
            require(
                result.length > 0,
                "NewTestModule: callDapp2 returned no result"
            );
            ...
            require(
                result.length == 0,
                "NewTestModule: callDapp2 returned some result"
            );
        }
    }
    ...
    function fail(address _wallet, string calldata reason) external {
        invokeWallet(
            ...
            address(dapp),
            0,
            abi.encodeWithSignature("doFail(string)", reason)
        );
    }
}
