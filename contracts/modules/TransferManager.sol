    ...
    LimitManager
{
    ...
    bytes4 private constant ERC1271_ISVALIDSIGNATURE_BYTES = bytes4(
        keccak256("isValidSignature(bytes,bytes)")
    );
    bytes4 private constant ERC1271_ISVALIDSIGNATURE_BYTES32 = bytes4(
    ...
    function init(BaseWallet _wallet) public onlyWallet(_wallet) {
        ...
        (uint256 pending, uint64 changeAfter) = oldLimitManager.getPendingLimit(
            _wallet
        );
        ...
        if (current == pending) {
            ...
            limits[address(_wallet)].limit = Limit(
                uint128(current),
                uint128(pending),
                changeAfter
            );
        ...
        (uint256 unspent, uint64 periodEnd) = oldLimitManager.getDailyUnspent(
            _wallet
        );
        // solium-disable-next-line security/no-block-members
        if (periodEnd > now) {
            limits[address(_wallet)].dailySpent = DailySpent(
                uint128(current.sub(unspent)),
                periodEnd
            );
        }
    }
    ...
    ) external onlyWalletOwner(_wallet) onlyWhenUnlocked(_wallet) {
        if (isWhitelisted(_wallet, _to)) {
            // transfer to whitelist
            doTransfer(_wallet, _token, _to, _amount, _data);
        } else {
            uint256 etherAmount = (_token == ETH_TOKEN)
                ? _amount
                : priceProvider.getEtherValue(_amount, _token);
            if (checkAndUpdateDailySpent(_wallet, etherAmount)) {
                // transfer under the limit
                doTransfer(_wallet, _token, _to, _amount, _data);
            } else {
                // transfer above the limit
                (bytes32 id, uint256 executeAfter) = addPendingAction(
                    ActionType.Transfer,
                    _wallet,
                    _token,
                    _to,
    ...
    function approveToken(
        ...
        if (isWhitelisted(_wallet, _spender)) {
            ...
            uint256 currentAllowance = ERC20(_token).allowance(
                address(_wallet),
                _spender
            );
            if (_amount <= currentAllowance) {
                ...
                uint256 delta = _amount - currentAllowance;
                uint256 deltaInEth = priceProvider.getEtherValue(delta, _token);
                require(
                    checkAndUpdateDailySpent(_wallet, deltaInEth),
    ...
    function callContract(
        ...
        if (isWhitelisted(_wallet, _contract)) {
            ...
            require(
                checkAndUpdateDailySpent(_wallet, _value),
                "TM: Call contract above daily limit"
            );
     ...
     */
    function addToWhitelist(BaseWallet _wallet, address _target)
        external
     ...
     */
    function removeFromWhitelist(BaseWallet _wallet, address _target)
        external
        onlyWalletOwner(_wallet)
        onlyWhenUnlocked(_wallet)
    {
    ...
    function executePendingTransfer(
        ...
        bytes calldata _data,
        uint256 _block
    ) external onlyWhenUnlocked(_wallet) {
        ...
        uint256 executeAfter = configs[address(_wallet)].pendingActions[id];
        require(executeAfter > 0, "TT: unknown pending transfer");
        uint256 executeBefore = executeAfter.add(securityWindow);
        // solium-disable-next-line security/no-block-members
        require(
            executeAfter <= now && now <= executeBefore,
    ...
    function cancelPendingTransfer(BaseWallet _wallet, bytes32 _id)
        external
        onlyWalletOwner(_wallet)
        onlyWhenUnlocked(_wallet)
    {
        require(
            configs[address(_wallet)].pendingActions[_id] > 0,
            "TT: unknown pending action"
        );
     ...
     */
    function changeLimit(BaseWallet _wallet, uint256 _newLimit)
        external
        onlyWalletOwner(_wallet)
        onlyWhenUnlocked(_wallet)
    {
     ...
     * @param _wallet The target wallet.
     */
    function disableLimit(BaseWallet _wallet)
        external
        onlyWalletOwner(_wallet)
        onlyWhenUnlocked(_wallet)
    {
     ...
     */
    function isWhitelisted(BaseWallet _wallet, address _target)
        public
        view
        returns (bool _isWhitelisted)
    {
        uint256 whitelistAfter = transferStorage.getWhitelist(_wallet, _target);
        // solium-disable-next-line security/no-block-members
     ...
     */
    function getPendingTransfer(BaseWallet _wallet, bytes32 _id)
        external
        view
        returns (uint64 _executeAfter)
    {
    ...
    function addPendingAction(
        ...
        uint256 _amount,
        bytes memory _data
    ) internal returns (bytes32 id, uint256 executeAfter) {
        id = keccak256(
            abi.encodePacked(_action, _token, _to, _amount, _data, block.number)
        );
        require(
            configs[address(_wallet)].pendingActions[id] == 0,
            "TM: duplicate pending action"
        );
        // solium-disable-next-line security/no-block-members
     ...
     * @param _contract The address of the contract.
     */
    function authoriseContractCall(BaseWallet _wallet, address _contract)
        internal
        view
    {
        require(
            _contract != address(_wallet) && // not the wallet itself
                ...
                (priceProvider.cachedPrices(_contract) == 0 ||
