...
contract GuardianManager is BaseModule, RelayerModule {
    ...
    bytes4 internal constant CONFIRM_ADDITION_PREFIX = bytes4(
        keccak256("confirmGuardianAddition(address,address)")
    );
    bytes4 internal constant CONFIRM_REVOKATION_PREFIX = bytes4(
        keccak256("confirmGuardianRevokation(address,address)")
    );
        ...
        onlyWhenUnlocked(_wallet)
    {
        ...
        (bool success, ) = _guardian.call.gas(5000)(
        ...
        if (guardianStorage.guardianCount(_wallet) == 0) {
            ...
            bytes32 id = keccak256(
                abi.encodePacked(address(_wallet), _guardian, "addition")
            );
            GuardianManagerConfig storage config = configs[address(_wallet)];
            require(
                config.pending[id] == 0 ||
                    now > config.pending[id] + securityWindow,
     ...
     */
    function confirmGuardianAddition(BaseWallet _wallet, address _guardian)
        public
        onlyWhenUnlocked(_wallet)
    {
        bytes32 id = keccak256(
     ...
     * @param _guardian The guardian.
     */
    function cancelGuardianAddition(BaseWallet _wallet, address _guardian)
        public
        onlyWalletOwner(_wallet)
        onlyWhenUnlocked(_wallet)
    {
        ...
        onlyWalletOwner(_wallet)
    {
        ...
        bytes32 id = keccak256(
            abi.encodePacked(address(_wallet), _guardian, "revokation")
        );
        GuardianManagerConfig storage config = configs[address(_wallet)];
        require(
            config.pending[id] == 0 ||
     ...
     * @param _guardian The guardian.
     */
    function confirmGuardianRevokation(BaseWallet _wallet, address _guardian)
        public
    {
        bytes32 id = keccak256(
            abi.encodePacked(address(_wallet), _guardian, "revokation")
     ...
     * @param _guardian The guardian.
     */
    function cancelGuardianRevokation(BaseWallet _wallet, address _guardian)
        public
        onlyWalletOwner(_wallet)
        onlyWhenUnlocked(_wallet)
    {
    ...
    {
        (_isGuardian, ) = GuardianUtils.isGuardian(
     ...
     */
    function guardianCount(BaseWallet _wallet)
        external
        view
    ...
    function getRequiredSignatures(
        ...
        if (
            methodId == CONFIRM_ADDITION_PREFIX ||
            methodId == CONFIRM_REVOKATION_PREFIX
        ) {
            return 0;
        }
        return 1;
