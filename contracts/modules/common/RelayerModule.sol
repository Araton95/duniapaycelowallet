...
contract RelayerModule is BaseModule {
    ...
    function execute(
        ...
        require(
            checkAndUpdateUniqueness(_wallet, _nonce, signHash),
            "RM: Duplicate request"
        );
        require(
            verifyData(address(_wallet), _data),
            "RM: the wallet authorized is different then the target of the relayed data"
        );
        uint256 requiredSignatures = getRequiredSignatures(_wallet, _data);
        if ((requiredSignatures * 65) == _signatures.length) {
            if (
                verifyRefund(_wallet, _gasLimit, _gasPrice, requiredSignatures)
            ) {
                if (
     ...
     * @param _wallet The target wallet.
     */
    function getNonce(BaseWallet _wallet)
        external
        view
     ...
     * @param _nonce The nonce
     */
    function checkAndUpdateNonce(BaseWallet _wallet, uint256 _nonce)
        internal
        returns (bool)
    {
    ...
    */
    function verifyData(address _wallet, bytes memory _data)
        private
        pure
        returns (bool)
    {
