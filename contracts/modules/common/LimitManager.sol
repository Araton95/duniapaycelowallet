...
contract LimitManager is BaseModule {
    ...
    uint128 private constant LIMIT_DISABLED = uint128(-1); // 3.40282366920938463463374607431768211455e+38
    ...
    function changeLimit(
        ...
        uint128 current = (limit.changeAfter > 0 && limit.changeAfter < now)
            ? limit.pending
            : limit.current;
        ...
        emit LimitChanged(
            address(_wallet),
            _newLimit,
            uint64(now.add(_securityPeriod))
        );
    }
     ...
     * @param _securityPeriod The security period.
     */
    function disableLimit(BaseWallet _wallet, uint256 _securityPeriod)
        internal
    {
     ...
     */
    function getCurrentLimit(BaseWallet _wallet)
        public
        view
        returns (uint256 _currentLimit)
    {
        Limit storage limit = limits[address(_wallet)].limit;
        _currentLimit = uint256(
            currentLimit(limit.current, limit.pending, limit.changeAfter)
        );
    }
     ...
     */
    function isLimitDisabled(BaseWallet _wallet)
        public
        view
        returns (bool _limitDisabled)
    {
     ...
     * @param _wallet The target wallet.
     * @return the pending limit (in Ccfa) and the time at chich it will become effective.
     */
    function getPendingLimit(BaseWallet _wallet)
        external
        view
        returns (uint256 _pendingLimit, uint64 _changeAfter)
    {
        Limit storage limit = limits[address(_wallet)].limit;
        // solium-disable-next-line security/no-block-members
        return (
            (now < limit.changeAfter)
                ? (uint256(limit.pending), limit.changeAfter)
                : (0, 0)
        );
    }
     ...
     */
    function getDailyUnspent(BaseWallet _wallet)
        external
        view
        returns (uint256 _unspent, uint64 _periodEnd)
    {
     ...
     */
    function checkAndUpdateDailySpent(BaseWallet _wallet, uint256 _amount)
        internal
        returns (bool)
    {
        if (_amount == 0) return true;
        Limit storage limit = limits[address(_wallet)].limit;
        uint128 current = currentLimit(
            limit.current,
            limit.pending,
            limit.changeAfter
        );
    ...
    function isWithinDailyLimit(
        ...
        if (expense.periodEnd < now) {
            ...
            return (expense.alreadySpent + _amount <= _limit &&
                expense.alreadySpent + _amount >= expense.alreadySpent);
        }
    }
