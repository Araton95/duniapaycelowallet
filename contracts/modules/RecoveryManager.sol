...
contract RecoveryManager is BaseModule, RelayerModule {
    ...
    bytes4 internal constant EXECUTE_RECOVERY_PREFIX = bytes4(
        keccak256("executeRecovery(address,address)")
    );
    bytes4 internal constant FINALIZE_RECOVERY_PREFIX = bytes4(
        keccak256("finalizeRecovery(address)")
    );
    bytes4 internal constant CANCEL_RECOVERY_PREFIX = bytes4(
        keccak256("cancelRecovery(address)")
    );
    bytes4 internal constant EXECUTE_OWNERSHIP_TRANSFER_PREFIX = bytes4(
        keccak256("executeOwnershipTransfer(address,address)")
    );
    bytes4 internal constant FINALIZE_OWNERSHIP_TRANSFER_PREFIX = bytes4(
        keccak256("finalizeOwnershipTransfer(address)")
    );
    bytes4 internal constant CANCEL_OWNERSHIP_TRANSFER_PREFIX = bytes4(
        keccak256("cancelOwnershipTransfer(address)")
    );
    ...
    event OwnershipTransferExecuted(
        address indexed _wallet,
    ...
    modifier onlyWhenRecovery(BaseWallet _wallet) {
        require(
            recoveryConfigs[address(_wallet)].executeAfter > 0,
            "RM: there must be an ongoing recovery"
        );
        _;
    }
    ...
    modifier notWhenRecovery(BaseWallet _wallet) {
        require(
            recoveryConfigs[address(_wallet)].executeAfter == 0,
            "RM: there cannot be an ongoing recovery"
        );
        _;
    }
    ...
    modifier onlyWhenOwnershipTransfer(BaseWallet _wallet) {
        require(
            ownershipTransferConfigs[address(_wallet)].executeAfter > 0,
            "RM: there must be an ongoing ownership transfer"
        );
        _;
    }
    ...
    modifier notWhenOwnershipTransfer(BaseWallet _wallet) {
        require(
            now >
                ownershipTransferConfigs[address(_wallet)].executeAfter +
                    securityWindow,
    ...
    ) public BaseModule(_registry, _guardianStorage, NAME) {
        require(
     ...
     */
    function executeRecovery(BaseWallet _wallet, address _recovery)
        external
        onlyExecute
     ...
     * @param _wallet The target wallet.
     */
    function finalizeRecovery(BaseWallet _wallet)
        external
        onlyWhenRecovery(_wallet)
    {
        RecoveryConfig storage config = recoveryConfigs[address(_wallet)];
        require(
            uint64(now) > config.executeAfter,
            "RM: the recovery period is not over yet"
        );
     ...
     * @param _wallet The target wallet.
     */
    function cancelRecovery(BaseWallet _wallet)
        external
        onlyExecute
        onlyWhenRecovery(_wallet)
    {
     ...
     * @param _wallet The target wallet.
     */
    function getRecovery(BaseWallet _wallet)
        public
        view
        returns (address _address, uint64 _executeAfter, uint32 _guardianCount)
    {
        RecoveryConfig storage config = recoveryConfigs[address(_wallet)];
     ...
     */
    function executeOwnershipTransfer(BaseWallet _wallet, address _newOwner)
        external
        ...
        notWhenOwnershipTransfer(_wallet)
    {
        require(
            _newOwner != address(0),
            "RM: new owner address cannot be null"
        );


            OwnershipTransferConfig storage config
         = ownershipTransferConfigs[address(_wallet)];
        config.newOwner = _newOwner;
        config.executeAfter = uint64(now + securityPeriod);
        emit OwnershipTransferExecuted(
            address(_wallet),
            _newOwner,
            config.executeAfter
        );
     ...
     * @param _wallet The target wallet.
     */
    function finalizeOwnershipTransfer(BaseWallet _wallet)
        external
        onlyWhenUnlocked(_wallet)
        onlyWhenOwnershipTransfer(_wallet)
    {
         ...
         = ownershipTransferConfigs[address(_wallet)];
        require(
            config.executeAfter < now,
            "RM: Too early to confirm ownership transfer"
        );
        require(
            now < config.executeAfter + securityWindow,
            "RM: Too late to confirm ownership transfer"
        );
     ...
     * @param _wallet The target wallet.
     */
    function cancelOwnershipTransfer(BaseWallet _wallet)
        external
        onlyWalletOwner(_wallet)
        onlyWhenUnlocked(_wallet)
        onlyWhenOwnershipTransfer(_wallet)
    ...
    function validateSignatures(
        ...
        for (uint8 i = 0; i < _signatures.length / 65; i++) {
            ...
            if (i == 0 && isOwner(_wallet, signer)) {
                ...
                lastSigner = signer;
                (isGuardian, guardians) = GuardianUtils.isGuardian(
                    guardians,
                    signer
                );
                if (!isGuardian) {
    ...
    function getRequiredSignatures(BaseWallet _wallet, bytes memory _data)
        internal
        view
        returns (uint256)
    {
        ...
        if (methodId == CANCEL_RECOVERY_PREFIX) {
            return
                SafeMath.ceil(
                    recoveryConfigs[address(_wallet)].guardianCount + 1,
                    2
                );
        }
