...
contract DappStorage is Storage {
    ...
    function setMethodAuthorization(
        ...
        for (uint256 i = 0; i < _signatures.length; i++) {
            whitelistedMethods[address(
                _wallet
            )][_dapp][_contract][_signatures[i]] = _authorized;
        }
    }
     ...
     */
    function getMethodAuthorization(
        BaseWallet _wallet,
        address _dapp,
        address _contract,
        bytes4 _signature
        ...
        return
            whitelistedMethods[address(_wallet)][_dapp][_contract][_signature];
    }
}
